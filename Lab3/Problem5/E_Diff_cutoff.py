...
def lattice_scan(ecut):
    ...
    alat1 = 5.658
    alat2 = 5.678
    output1 = compute_energy(alat=alat1, ecut=ecut, nk=nk)
    output2 = compute_energy(alat=alat2, ecut=ecut, nk=nk)
    energy = output1["energy"] - output2["energy"]
    print(energy)
    return energy
...
if __name__ == "__main__":
    ...
    data = {
        "Cutoff (Ry)": cuts,
        "Energy Difference(eV)": energies,
        "Energy Difference(Ry)": energies_ry,
    ...
    df.to_csv('cutoff_difference_results.csv', index=False)
    print(df)
    plt.plot(cuts, energies, marker='o')
    plt.title('Cutoff Energy Difference')
    plt.xlabel('Cutoff Energy (Ry)')
    plt.ylabel('Total Energy Difference(eV)')
    plt.savefig('cutoff_difference_convergence.png')
    plt.show()
