...
def compute_energy(form, alat, nk, ecut, clat=None, anti=False, recalculate=False):
    ...
    dirname = "Fe_{}_nk_{}_relax".format(form, nk)
    runpath = Dir(path=os.path.join(os.environ["WORKDIR"], "Lab4/1A", dirname))
...
def lattice_scan(form, nk, anti=False):
    ...
    lat_dic = extract_lattice_parameters(output["cell_parameters"], form)
    return energy, lat_dic

if __name__ == "__main__":
    # put here the function that you actually want to run
    nks = [2, 4, 6, 8, 10, 12, 14, 16]
    bcc_energies = []
    bcc_alats = []
    hcp_energies = []
    hcp_alats = []
    hcp_clats = []
    for nk in nks:
        e, lat_dic = lattice_scan("bcc", nk)
        bcc_energies.append(e)
        bcc_alats.append(lat_dic["a"])
        e, lat_dic = lattice_scan("hcp", nk)
